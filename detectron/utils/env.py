...
def exit_on_error():
    """Exit from a Detectron-ZZnet tool when there's an error."""
    sys.exit(1)
...
def get_detectron_ops_lib():
    ...
    # Candidate prefixes for Detectron-ZZnet ops lib path
    prefixes = [_CMAKE_INSTALL_PREFIX, sys.prefix, sys.exec_prefix] + sys.path
    # Candidate subdirs for Detectron-ZZnet ops lib
    subdirs = ['lib', 'torch/lib']
    # Try to find Detectron-ZZnet ops lib
